/**
 * Raw note API response types (internal implementation details)
 */

export interface RawSubstackNote {
  entity_key: string
  type: string
  context: {
    type: string
  ...
  parentComments?: Array<RawSubstackNoteComment>
  canReply: boolean
  isMuted: boolean
  trackingParameters: {
...
export interface RawSubstackNoteComment {
  id: number
  body: string
  user_id: number
  name: string
...
export interface RawSubstackPublication {
  name: string
  hostname: string
  subdomain: string
  logo?: {
