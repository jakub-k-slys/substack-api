...
describe('SubstackClient - Slug Resolution', () => {
  ...
  describe('slug resolution via subscriptions cache', () => {
    ...
    it('should clear cache when refreshSlugCache is called', async () => {
      const mockProfile: SubstackFullProfile = {
        id: 123,
        name: 'Test User',
        handle: 'fallback-handle',
      ...
      mockHttpClient.get
        .mockResolvedValue(mockProfile) // /api/v1/users/123
        .mockResolvedValue(mockSubscriptionsResponse) // /api/v1/subscriptions

      // First call should cache subscriptions
      await client.profileForId(123)

      // Clear cache
      client.refreshSlugCache()

      // Second call should fetch subscriptions again
      await client.profileForId(123)

      // Should call subscriptions endpoint twice (once before and once after cache clear)
      expect(mockHttpClient.get).toHaveBeenCalledWith('/api/v1/subscriptions')
      expect(
        mockHttpClient.get.mock.calls.filter((call) => call[0] === '/api/v1/subscriptions')
      ).toHaveLength(2)
    })
  })
})
