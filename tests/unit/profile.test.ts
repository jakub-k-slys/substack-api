...
describe('Profile Entity', () => {
  ...
  describe('notes()', () => {
    ...
    it('should fall back to own notes endpoint when reader feed fails', async () => {
      ...
      // Setup mock to fail on reader feed, succeed on own notes
      mockHttpClient.get.mockImplementation((url: string) => {
        if (url.includes('/api/v1/reader/feed/profile/')) {
          return Promise.reject(new Error('Reader feed not available'))
        } else if (url === '/api/v1/notes') {
          return Promise.resolve(mockOwnNotesResponse)
        }
      ...
      expect(notes).toHaveLength(1)
      expect(notes[0].body).toBe('Own profile note')

      // Verify both endpoints were called: first the reader feed (which failed), then own notes
      expect(mockHttpClient.get).toHaveBeenCalledWith(
        '/api/v1/reader/feed/profile/123?types=note&limit=25&offset=0'
      )
